"""
    Intended as a counterpart to {DATABLOCKS}/test/test_dbx.py to be used outside of 'pytest' for debugging.
    For use from a python shell.
"""
...
from datablocks.dbx import DATALAKE



VALIDATION_LAKE = datablocks.DATALAKE.subspace('validation')
VALIDATION_FILE_POOL = datablocks.FILE_POOL.clone(dataspace=VALIDATION_LAKE, throw=True)
VERBOSE = True


MIRLOGCOHN = datablocks.DBX('datablocks.test.micron.datablocks.miRLogCoHN', 'mirlogcohn')\
            .Datablock(verbose=VERBOSE)\
            .Databuilder(dataspace=VALIDATION_LAKE, pool=VALIDATION_FILE_POOL, verbose=VERBOSE)\
...
MIRCOSEQSHN = datablocks.DBX('datablocks.test.micron.datablocks.miRCoSeqs', f'mircoseqshn_{MIR_COSEQS_NPASSES}_{MIR_COSEQS_SEQS_PER_RECORD}')\
...
BASH = """
export VALIDATION_LAKE="datablocks.DATALAKE.subspace('validation')"
export VALIDATION_FILE_POOL="datablocks.FILE_POOL.clone(dataspace=$VALIDATION_LAKE, throw=True)"
export VERBOSE="True"
export MIRLOGCOHN="datablocks.DBX('datablocks.test.micron.datablocks.miRLogCoHN', 'mirlogcohn')\
            .Datablock(verbose=$VERBOSE)\
            .Databuilder(dataspace=$VALIDATION_LAKE, pool=$VALIDATION_FILE_POOL, verbose=$VERBOSE)"
dbx "$MIRLOGCOHN"
export MIRCOHN="datablocks.DBX('datablocks.test.micron.datablocks.miRCoHN', 'mircohn')\
        .SCOPE(logcounts=$MIRLOGCOHN.READ())\
        .Databuilder(dataspace=$VALIDATION_LAKE, pool=$VALIDATION_FILE_POOL, verbose=$VERBOSE)";\
dbx "$MIRCOHN"
export MIRNA="datablocks.DBX('datablocks.test.micron.datablocks.miRNA', 'mirna')\
    .Datablock(verbose=$VERBOSE)\
    .Databuilder(dataspace=$VALIDATION_LAKE, pool=$VALIDATION_FILE_POOL, verbose=$VERBOSE)"
dbx "$MIRNA"
export MIR_COSEQS_NPASSES=10
export MIR_COSEQS_SEQS_PER_RECORD=300
export MIRCOSEQSHN="datablocks.DBX('datablocks.test.micron.datablocks.miRCoSeqs', f'mircoseqshn_${MIR_COSEQS_NPASSES}_${MIR_COSEQS_SEQS_PER_RECORD}')\
                .SCOPE(logcounts=$MIRCOHN.READ('logcounts'), \
                       logcontrols=$MIRCOHN.READ('logcontrols'),\
                       seqs=$MIRNA.READ(), \
                       npasses=$MIR_COSEQS_NPASSES, \
...
def test_pandas_datablock():
    pdbk = datablocks.DBX('datablocks.test.pandas.datablocks.PandasArray', 'pdbk')\
        .Databuilder(dataspace=VALIDATION_LAKE)
...
def test_pandas_build_exception():
    pexc = datablocks.DBX('datablocks.test.pandas.datablocks.BuildExceptionDatablock', 'pexc')\
            .Databuilder(dataspace=VALIDATION_LAKE, pool=VALIDATION_FILE_POOL)
